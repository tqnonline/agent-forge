...
### Changed
- `release.yml` rewritten to use inline shell for PyPI publish (OIDC + twine)
  and GitHub Release creation (`gh release create`). Removes dependency on
  `pypa/gh-action-pypi-publish` and `softprops/action-gh-release` — both
  failed to download during v1.1.0 cut due to a GitHub codeload regional
  outage. Inline shell uses only first-party actions (`actions/checkout`,
